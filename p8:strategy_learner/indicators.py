...
def sma(prices, lookback, symbol, make_plot):
    ...
    sma_normalized.rename(columns={symbol:'SMA_' + symbol}, inplace=True)
...
def bbp(prices, lookback, symbol, make_plot):
    ...
    bbp_df.rename(columns={symbol:'Bollinger Band %'}, inplace=True)

    bb_df = prices.copy()
    bb_df.rename(columns={symbol:symbol + ' Price'}, inplace=True)
    bb_df['SMA'] = sma_df
    ...
    if make_plot:
        ...
        ax1.set_title('Bollinger Bands Against ' + symbol + ' Price Data')
        ax1.legend(prop={'size':'10'}, loc=4)
        ax1.set(ylabel='$ Value')

        ax2.set_title('Bollinger Band % for ' + symbol)
...
def macd(prices, symbol, make_plot):
    ...
    macd_df.rename(columns={symbol:'MACD'}, inplace=True)
